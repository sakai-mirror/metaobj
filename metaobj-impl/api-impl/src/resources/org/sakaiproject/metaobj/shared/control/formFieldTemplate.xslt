...
<xsl:stylesheet version="2.0" xmlns="http://www.w3.org/1999/xhtml"
	...
	xmlns:osp="http://www.osportfolio.org/OspML" xmlns:xs="http://www.w3.org/2001/XMLSchema">
	...
	<xsl:template name="complexElement-field">
		...
		<div class="act">
			<input type="submit" name="addButton" alignment="center" value="Add New"
				onClick="this.form.childPath.value='{$name}';return true"/>
		</div>
	</xsl:template>
				...
				<tr>
					<td>
						...
						<xsl:if test="string($currentNode) = ''">
							<script language="JavaScript" type="text/javascript"> document.forms[0].<xsl:value-of
									select="$name"/>.value="" </script>
						</xsl:if>
		...
		<div class="shorttext">
			...
			<a>
				<xsl:attribute name="href">javascript:document.forms[0].childPath.value='<xsl:value-of
						select="$name"
					/>';document.forms[0].fileHelper.value='true';document.forms[0].onsubmit();document.forms[0].submit();</xsl:attribute>
	...
	<xsl:template name="calendar-widget">
		...
		<input type="text" size="10" title="mm/dd/yyyy" name="{$schemaNode/@name}.fullDate"
			...
			<xsl:attribute name="value">
				<xsl:if test="$year > -1"><xsl:value-of select="$month"/>/<xsl:value-of select="$day"
						/>/<xsl:value-of select="$year"/></xsl:if>
			</xsl:attribute>
		</input>
		<img width="16" height="16" style="cursor:pointer;" border="0"
			...
			<xsl:attribute name="onclick">javascript:var cal<xsl:value-of select="$fieldId"/> = new
					calendar2(document.getElementById('<xsl:value-of select="$schemaNode/@name"
					/>'));cal<xsl:value-of select="$fieldId"/>.year_scroll = true;cal<xsl:value-of
					select="$fieldId"/>.time_comp = false;cal<xsl:value-of select="$fieldId"
				/>.popup('','/sakai-jsf-resource/inputDate/')</xsl:attribute>
		</img>
	...
	<xsl:template name="subListRow">
		...
		<tr>
			...
			<td class="itemAction">
				<!-- todo: (gsilver) see if can avt this blob -->
				<a>
					<xsl:attribute name="href">javascript:document.forms[0].childPath.value='<xsl:value-of
							select="$fieldName"
							/>';document.forms[0].editButton.value='Edit';document.forms[0].removeButton.value='';document.forms[0].childIndex.value='<xsl:value-of
							select="$index"/>';document.forms[0].onsubmit();document.forms[0].submit();</xsl:attribute>
					edit </a> | <a>
					<xsl:attribute name="href">javascript:document.forms[0].childPath.value='<xsl:value-of
							select="$fieldName"
							/>';document.forms[0].removeButton.value='Remove';document.forms[0].editButton.value='';document.forms[0].childIndex.value='<xsl:value-of
							select="$index"/>';document.forms[0].onsubmit();document.forms[0].submit();</xsl:attribute>
					remove </a>
			</td>
